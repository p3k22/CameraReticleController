namespace P3k.CameraReticleController.Components
{
   ...
   public class ReticleController : MonoBehaviour, IReticleController
   {
      ...
      /// <summary>
      ///    Triggers the animation for the active reticle, but only if the
      ///    current target's <see cref="ReticleType" /> matches <paramref name="type" />.
      ///    This allows different inputs to call animate independently; only the
      ///    input whose type corresponds to the current target will actually fire.
      /// </summary>
      public void AnimateReticle(ReticleType type)
      {
         if (!_allowReticles || CurrentTarget == null)
         {
            return;
         }

         if (CurrentTarget.ReticleType != type)
         {
            return;
         }

         if (_reticleLookup.TryGetValue(CurrentTarget.ReticleTag, out var reticleRef))
         {
